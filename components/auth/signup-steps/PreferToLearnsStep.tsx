...
import { Video, FileText, Code, Laptop, ArrowRight, ArrowLeft, Loader2, Sparkles, Headphones, PenTool } from "lucide-react"
    ...
    return (
        ...
        >
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-3">
                    <div className="inline-flex items-center gap-2 rounded-full border border-white/40 bg-white/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-500 dark:bg-white/10 dark:text-slate-200">
                        <Headphones className="h-3.5 w-3.5 text-pink-500" /> Step 5 · Style
                    </div>
                    <h2 className="text-2xl sm:text-3xl font-semibold leading-tight bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 bg-clip-text text-transparent">
                        How do you want your lessons to feel?
                    </h2>
                    <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 max-w-2xl">
                        This shapes the media mix we surface—hands-on projects, immersive videos, quick reads, or collaborative labs.
                    </p>
                </div>
                <motion.div
                    ...
                    return (
                        <motion.button
                            ...
                            className={`relative flex h-24 w-full flex-col justify-between overflow-hidden rounded-3xl border border-white/50 bg-white/80 p-5 text-left shadow-md transition hover:shadow-xl dark:bg-white/10 ${
                                isSelected ? "ring-2 ring-pink-500/60" : ""
                            }`}
                        >
                            <div className="absolute inset-0 opacity-0 transition group-hover:opacity-100" style={{ background: "linear-gradient(140deg, rgba(236,72,153,0.15) 0%, rgba(59,130,246,0.12) 100%)" }} />
                            <div className="relative flex items-center gap-3">
                                <div className={`flex h-11 w-11 items-center justify-center rounded-xl text-white shadow-lg transition ${isSelected ? "bg-gradient-to-br from-pink-500 to-purple-500" : "bg-gradient-to-br from-slate-100 to-slate-200 text-slate-500"}`}>
                                    <IconComponent className="h-5 w-5" />
                                </div>
                                <div className="flex-1">
                                    <p className="text-base font-semibold text-slate-800 dark:text-white">{style.name}</p>
                                    <p className="text-xs text-slate-500 dark:text-slate-300">We’ll prioritize matching content formats</p>
                                </div>
                                <AnimatePresence>
                                    {isSelected && (
                                        <motion.div
                                            ...
                                            animate={{ scale: 1 }}
                                            exit={{ scale: 0 }}
                                            className="flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-pink-500 to-purple-500 text-white"
                                        >
            ...
            <div className="mt-6 grid gap-3 text-xs text-slate-500 dark:text-slate-300 sm:grid-cols-3">
                {["Determines lesson pacing", "Helps curate facilitators", "Optimizes notifications"].map((item) => (
