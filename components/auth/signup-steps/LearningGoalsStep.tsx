...
import { Briefcase, Sparkles, Heart, GraduationCap, ArrowRight, ArrowLeft, Loader2, Target, Lightbulb } from "lucide-react"
    ...
    return (
        ...
        >
            <motion.div
                ...
                style={{
                    background:
                        "radial-gradient(circle at 12% 20%, rgba(59,130,246,0.13) 0%, transparent 55%), radial-gradient(circle at 88% 15%, rgba(236,72,153,0.15) 0%, transparent 60%), radial-gradient(circle at 50% 85%, rgba(124,58,237,0.18) 0%, transparent 60%)",
                }}
            />
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-3">
                    <div className="inline-flex items-center gap-2 rounded-full border border-white/40 bg-white/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.3em] text-slate-500 dark:bg-white/10 dark:text-slate-200">
                        <Target className="h-3.5 w-3.5 text-blue-500" /> Step 4 · Goals
                    </div>
                    <h2 className="text-2xl sm:text-3xl font-semibold leading-tight bg-gradient-to-r from-blue-600 via-purple-600 to-pink-500 bg-clip-text text-transparent">
                        What outcomes are you aiming for right now?
                    </h2>
                    <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 max-w-2xl">
                        Choose every goal that resonates. We use this to tailor course suggestions, coaching invites, and free plan recommendations.
                    </p>
                </div>
                <motion.div
                        ...
                        >
                            ...
                            <div className="relative flex items-center gap-3">
                                ...
                                <div className="flex-1">
                                    <p className="text-base font-semibold text-slate-800 dark:text-white">{goal.name}</p>
                                    <p className="text-xs text-slate-500 dark:text-slate-300">Boosts tailored recommendations</p>
                                </div>
                                <AnimatePresence>
                                    {isSelected && (
                                        <motion.div
                ...
                >
                    <span className="relative z-10 flex items-center gap-2 text-sm font-semibold">
                        {isLoading ? (
                            ...
                            <>
                                Next: Learning Style <ArrowRight className="h-4 w-4" />
                            </>
                        )}
                    </span>
                    <motion.div
            ...
            <div className="mt-6 grid gap-3 text-xs text-slate-500 dark:text-slate-300 sm:grid-cols-3">
                {["Improves course matching", "Influences onboarding tips", "Customizes dashboard analytics"].map((item) => (
