...
export default function CourseDetailPage() {
    ...
    const [previewUrl, setPreviewUrl] = useState("https://res.cloudinary.com/dpf3zv351/video/upload/v1761111084/elearning-into_mrqwdg.mp4")

    const course = {
        id: "1",
        title: "Build Text to Image SaaS App in React JS",
        description: "Master MERN Stack by building a Full Stack AI Text to Image SaaS App using React js, MongoDB, Node js, Express js and Stripe Payment",
        ...
        courseStructure: [
            {
                ...
                lessons: [
                    ...
                    { title: "Core Features - Authentication, payment, deployment", duration: "20 mins", completed: false },
                ],
            },
            // ... (other sections remain unchanged)
        ],
        description_full: "This is the most comprehensive and in-depth JavaScript course with 50 JavaScript projects...",
        comments: [
            { id: 1, user: "Good lesson!", rating: 5, comment: "This course is the best course that I ever seen before!", date: "2 days ago" },
            { id: 2, user: "This course is the best course that I ever seen before!", rating: 5, comment: "", date: "3 days ago" },
            { id: 3, user: "Good lesson!", rating: 4, comment: "", date: "5 days ago" },
        ],
        isSale: true,
        isFree: false,
    }
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                <div className="flex items-center gap-2">
                                    <Avatar className="w-10 h-10">
                                        ...
                                        <AvatarFallback>{course.educator.name.split(" ").map((n) => n[0]).join("")}</AvatarFallback>
                                    </Avatar>
                                    <div>
                                        <p className="font-semibold">
                                            Course by{" "}
                                            <Link href={`/users/${course.educator.id}`} className="text-primary hover:underline cursor-pointer">
                                                    ...
                                                    {section.lessons.map((lesson, lessonIndex) => (
                                                        <div key={lessonIndex} className="p-4 border-b last:border-b-0 flex items-center gap-3">
                                                            <div className="w-6 h-6 rounded-full border-2 border-muted-foreground/30 flex items-center justify-center">
                                                                {lesson.completed ? <CheckCircle className="w-4 h-4 text-green-500" /> : <Play className="w-3 h-3 text-muted-foreground" />}
                                                            </div>
                                        ...
                                        <div key={comment.id} className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                <Avatar className="w-8 h-8">
                                                    <AvatarFallback className="text-xs">{comment.user.split(" ").map((n) => n[0]).join("")}</AvatarFallback>
                                                </Avatar>
                                                <div className="flex-1">
                                            ...
                                            {comment.comment && <p className="text-sm text-muted-foreground ml-10">{comment.comment}</p>}
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className="absolute inset-0 bg-black/20 rounded-t-lg flex items-center justify-center">
                                            ...
                                            <CoursePreviewModal
                                                ...
                                                fileType={"video"}
                                                isCourseRestricted={course.isSale || course.isFree}
                                                onUrlChange={handleUrlChange}
                                            />
                                        </div>
                                        {course.discount && <Badge className="absolute top-4 right-4 bg-red-500 text-white">{course.discount}</Badge>}
                                    </div>
                                </CardHeader>
                                <CardContent className="p-6 space-y-6">
                                    <div className="text-center">
                                        ...
                                        <div className="mt-3 flex gap-3 justify-center items-center content-container">
                                            ...
                                            <CourseActionsDropdown courseId={course.id} courseTitle={course.title} />
                                        </div>
                                    </div>
                                    <Separator />
                                    <div>
